...
class RegistrationView(BaseView):
    ...
    def __init__(self, root, registration_login_frame):
        ...
        self.root.title('רישום')
        self.__init_widgets()

    def __register(self):
        adaptive_learning = AdaptiveLearning.get_instance()
        try:
            result = adaptive_learning.register_student(self.username_stringVar.get(), self.password_stringVar.get())
    ...
    def __init_widgets(self):
        ...
        topic_label = TopicLabel(self.topic_frame, "רישום").get_label()
        topic_label.pack()
        ...
        button_registration = PrimaryButton(self.buttons_frame, "הירשם", 'white', 15, self.__register).get_button()
